...
export const GetBikesCollector = async (citiesId: number[] | null, statusValue: string[] | null, tagsValue: string[] | null, lastUpdate: number | null = null) => {
    let clauseCount = 0;
    ...
    const response = await getSequelize().query<BikeTechnician>(`
        ...
        select b.*,
               ...
               l.uuid                                              AS lock_uuid,
               'BIKE'                                              as marker_type,
               'COLLECTED' = ANY (b.tags)                          as reported
        from bikes b
