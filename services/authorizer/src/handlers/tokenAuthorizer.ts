...
import {JWT_SECRET, JWT_SECRET_ADMIN} from "../config/config";
...
export const handler = async (event: APIGatewayRequestAuthorizerEventV2 | APIGatewayRequestAuthorizerEvent) => {
    ...
    const token = origin === "TECH_APP" || origin === "ADMIN_APP" ? JWT_SECRET_ADMIN : JWT_SECRET;
    const verifiedToken = verifyToken(bearerToken, token);
