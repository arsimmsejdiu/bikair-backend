 ...
 */
export const GetPolygons = async (query: SelectBuilderConf | null) => {
    ...
    const response = await getSequelize().query<GetCityPolygonsOutputData>(`
        SELECT cp.*,
               c.name AS city_name
        from city_polygons cp
                 LEFT OUTER JOIN cities c ON cp.id = c.id
            ${queryString}
